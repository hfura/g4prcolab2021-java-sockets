...
/*
 * This client thread class handles individual clients in their respective
 * threads
...
class clientThread extends Thread {
  ...
  /****
   * This function transfers message or files to all the client except a
   * particular client connected to the server
   ***/
      ...
      synchronized (this) {
        for (clientThread curr_client : clients) {
          if (curr_client != null && curr_client != this && curr_client.clientName != null
            ...
            curr_client.os.writeObject("Sending_File:"
                + words[1].split(" ", 2)[1].substring(words[1].split("\\s", 2)[1].lastIndexOf(File.separator) + 1));
        ...
        this.os.flush();
        System.out
            .println("File sent by " + this.clientName.substring(1) + " to everyone except " + words[0].substring(1));
      }
    }
      ...
      if (words.length > 1 && words[1] != null) {
        ...
        if (!words[1].isEmpty()) {
          synchronized (this) {
            ...
            this.os.flush();
            System.out.println(
                "Message sent by " + this.clientName.substring(1) + " to everyone except " + words[0].substring(1));
          }
        }
      }
    }
  ...
  /****
   * This function transfers message or files to all the client connected to the
   * server
   ***/
      ...
      synchronized (this) {
        for (clientThread curr_client : clients) {
          if (curr_client != null && curr_client.clientName != null && curr_client.clientName != this.clientName) {
            curr_client.os.writeObject("Sending_File:"
                + line.split("\\s", 2)[1].substring(line.split("\\s", 2)[1].lastIndexOf(File.separator) + 1));
            curr_client.os.writeObject(file_data);
            curr_client.os.flush();
    ...
    if (words[1].split(" ")[0].toLowerCase().equals("sendfile")) {
      ...
      for (clientThread curr_client : clients) {
        if (curr_client != null && curr_client != this && curr_client.clientName != null
          ...
          curr_client.os.writeObject(file_data);
          curr_client.os.flush();
          System.out.println(this.clientName.substring(1) + " transferred a private file to client "
              + curr_client.clientName.substring(1));
        ...
        if (!words[1].isEmpty()) {
          ...
          for (clientThread curr_client : clients) {
            if (curr_client != null && curr_client != this && curr_client.clientName != null
              ...
              System.out.println(this.clientName.substring(1) + " transferred a private message to client "
                  + curr_client.clientName.substring(1));
